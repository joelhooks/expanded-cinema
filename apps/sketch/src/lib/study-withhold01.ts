...
/**
 * Study: withhold-01 — the image withholds itself. Branches clock-04 v18
...
function buildScene() {
  ...
  const screenMat = new THREE.MeshBasicMaterial({
    map: null, // videoTexture assigned at the SHUTTER, never before
    color: 0x05060a, // withheld: an authored near-black plate
  ...
  // withhold01.luma.stats). A 2D canvas tap is synchronous and honest: draw
...
export async function mountRuntime(
  ...
  return await sketchEvents.measured("study", "study.mount", { study: STUDY }, async () => {
    ...
    const openAperture = (at: number, via: "seeked" | "backstop"): void => {
      ...
      void sketchEvents
        .emitInfo("study", "withhold01.aperture", { at, via })
        .catch(() => undefined);
    };
    ...
    const revealProofTick = (elapsed: number): void => {
      ...
      const vid = vidOf();
      void sketchEvents
        .emitInfo("study", "withhold01.arm.proof", {
    ...
    return {
      ...
      onPointer(p) {
        ...
        void sketchEvents
          .emitInfo("study", "withhold01.intervention", { sha: p.sha.slice(0, 16), source: "pointer" })
          .catch(() => undefined);
      },
      step(r, now, delta) {
        ...
        if (tap) {
          ...
          if (lumaFramesSeen % 600 === 1) {
            ...
            void sketchEvents
              .emitInfo("study", "withhold01.luma.stats", {
                seen: lumaFramesSeen,
          ...
          if (detector.push(frame)) {
            study.traceAt = performance.now();
            void sketchEvents
              .emitInfo("study", "withhold01.cut", { at: Number(study.traceAt.toFixed(0)), source: "film" })
              .catch(() => undefined);
          }
        }
        ...
        if (bubbleLogs < 10) {
          ...
          if (Math.floor(elS) > bubbleLogs) {
            ...
            void sketchEvents
              .emitInfo("study", "withhold01.beam.alpha", {
                s: bubbleLogs,
                cam: [
