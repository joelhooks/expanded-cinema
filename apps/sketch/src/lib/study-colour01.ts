...
function buildScene() {
  ...
  // colour01.luma.stats). A 2D canvas tap is synchronous and honest: draw
...
export async function mountRuntime(
  ...
  return await sketchEvents.measured("study", "study.mount", { study: STUDY }, async () => {
    ...
    const revealProofTick = (elapsed: number): void => {
      ...
      const vid = vidOf();
      void sketchEvents
        .emitInfo("study", "colour01.arm.proof", {
    ...
    return {
      ...
      onPointer(p) {
        ...
        void sketchEvents
          .emitInfo("study", "colour01.intervention", { sha: p.sha.slice(0, 16), source: "pointer" })
          .catch(() => undefined);
      },
      step(r, now, delta) {
        ...
        if (tap) {
          ...
          if (lumaFramesSeen % 600 === 1) {
            ...
            void sketchEvents
              .emitInfo("study", "colour01.luma.stats", {
                seen: lumaFramesSeen,
          ...
          if (detector.push(frame)) {
            study.traceAt = performance.now();
            void sketchEvents
              .emitInfo("study", "colour01.cut", { at: Number(study.traceAt.toFixed(0)), source: "film" })
              .catch(() => undefined);
          }
        }
        ...
        if (!railAudit.reported && elapsed > 48_000) {
          ...
          void sketchEvents
            .emitInfo("study", "colour01.rail.audit", {
        ...
        if (bubbleLogs < 10) {
          ...
          if (Math.floor(elS) > bubbleLogs) {
            ...
            void sketchEvents
              .emitInfo("study", "colour01.beam.alpha", {
                s: bubbleLogs,
                cam: [
