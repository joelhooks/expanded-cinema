...
> = {
  ...
  "melt-01": () => import("./lib/study-melt01"),
};
...
try {
  ...
  // melt-01 app bundle: while this sha is the one being verified, the
  // page mounts THE STUDY ITSELF when no pointer names it (the gallery
  // pointer still serves colour-01 by design — cuts write only from
  // verified builds). Verification loads hit the archive URL directly;
  // dev/dev-server opens default to the app's own study.
  const FALLBACK_POINTER: CurrentStudy = {
    study: "melt-01",
    sha: "bundle",
    updatedAt: "1970-01-01T00:00:00Z",
    url: "https://cinema.wzrrd.sh/",
    archive: "/archive/melt-01/67a7c34/",
  };
  ...
  // Verification and study isolation: an archive URL carries its own study
  // in the path (/archive/<study>/<sha>/). When the page is served FROM an
  // archive build, mount THAT study regardless of the live gallery
  // pointer — otherwise every build verify just re-mounts the current
  // gallery study and no sha can ever be judged on its own terms.
  const fromArchive = /^\/archive\/([\w-]+)\//.exec(window.location.pathname);
  const booted = fromArchive
    ? ({ ...FALLBACK_POINTER, study: fromArchive[1] ?? FALLBACK_POINTER.study })
    : ((await fetchCurrent()) ?? FALLBACK_POINTER);
  // r7 pace: when the archive path names a study, resolve its module FIRST
  // and boot the video layer directly on that study's own material — the
  // boot-then-swap dance cost a full second cold element load, which
  // pushed the aperture past the verify window on cold caches.
  if (fromArchive) {
    try {
      const mod = await (STUDY_LOADERS[booted.study]?.() ?? Promise.reject(new Error("no loader")));
      if (mod.VIDEO?.src) {
        layer = await openVideoLayer(mod.VIDEO).catch(() => null);
      }
    } catch {
      // fall through to the default boot; the mount path still handles the swap
    }
  }
  layer = layer ?? (await openVideoLayer().catch(() => null));
