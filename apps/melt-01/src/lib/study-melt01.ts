...
/**
 * Study: surface/melt-01 v01 — the facet heap melts under the film's own
 * luma. Branches wallform-01 (research/2026-09-15-melt-01.md, keyed to the
 * psychedelic successor order via direction.svx 2026-09-16). Proven bijou
 * scene carried verbatim from withhold/wallform; the ONE changed thing:
 * the shard heap carries a trail buffer (N=8 frames) of picture motion —
 * luma delta + cut detonation — and the facets OOZE (sink, droop, spread)
 * while footage moves and HEAL toward base when still. v01 realizes the
 * feedback-displacement as per-facet transforms (honest deviation from the
 * TSL material spec; the material itself is queued as v02). Subtraction
 * test: with the melt block inert the page reduces to wallform's scene.
 */
...
/** Full scene-side runtime returned by buildScene (structural superset,
 * deliberately NOT the deliverable interface). */
type SceneRuntime = {
  shards: THREE.Mesh[];
...
/**
 * melt-01 v01 trail: per-facet activations over an N=8-frame history of
 * picture motion. Excursion clamp: max sink 0.55 (0.35 of heap height
 * 1.9 — the spec's guard so a bone-white cut cannot tear facets out of
 * the heap), no teleport, rotation bounded. Stillness decays activation
 * toward heal.
 */
const MELT_N = 8;
...
const meltFacets: MeltFacet[] = [];
let meltTrail: number[] = [];
let meltActivationsRaised = false;

function meltTick(
  study: SceneRuntime,
  video: HTMLVideoElement | undefined,
  tap: { luma: Float32Array } | null,
  revealed: boolean,
  n: number,
  detector: InstanceType<typeof CutDetector>,
): void {
  if (!revealed) return;
  if (meltFacets.length === 0) {
    for (const m of study.shards) {
      meltFacets.push({
        a: 0,
        responsiveness: 0.7 + ((m.userData.uv0?.x ?? 0) % 1) * 0.8,
      });
    }
  }
  // picture motion energy at frame pace (~every 2nd frame keeps 30 shards cheap)
  if (tap && video && video.readyState >= 2 && n % 2 === 0) {
    const prev = study.lumaAt(MELT_N);
    if (prev && prev.length === tap.luma.length) {
      let delta = 0;
      for (let i = 0; i < tap.luma.length; i += 4) {
        delta += Math.abs((tap.luma[i] ?? 0) - (prev[i] ?? 0));
      }
      const energy = delta / (tap.luma.length / 4);
      meltTrail.push(energy);
      if (meltTrail.length > MELT_N) meltTrail.shift();
    }
  } else {
    meltTrail.push(0);
    if (meltTrail.length > MELT_N) meltTrail.shift();
  }
  const motion = meltTrail.reduce((acc, v) => acc + v, 0) / Math.max(1, meltTrail.length);
  ...
  study.shards.forEach((m, i) => {
    ...
    const baseRot = m.userData.baseRot as THREE.Euler;
    if (!base || !baseRot) return;
    // drive: luma motion raises activation; cut frames spike it.
    // Activation decays fast (0.86/frame half-life ~4 frames at 30fps);
    // sustained motion keeps it up, a passed cut leaves a fading ooze.
    const drive = motion * 2.4 * f.responsiveness + cutPulse * 1.6 * f.responsiveness;
    f.a = Math.min(1, Math.max(f.a * 0.86, drive));
    if (f.a > 0.02 && !meltActivationsRaised) meltActivationsRaised = true;
    // ooze: sink + droop; heal returns home. Bounds tuned so the heap
    // never leaves its floor print (apparatus legibility, floor line 4).
    const sink = Math.min(MELT_MAX_SINK, Math.pow(f.a, 1.4) * MELT_MAX_SINK);
    m.position.y = base.y - sink * 0.55;
    m.position.x = base.x + f.a * 0.16 * (base.x > HEAP_DRIFT_X ? -0.5 : 0.9) * (i % 2 === 0 ? 1 : -1);
    m.position.z = base.z + f.a * 0.1 * (i % 3 === 0 ? 1 : -1);
    m.rotation.x = baseRot.x + f.a * 0.55; // droop toward camera-forward
    m.rotation.z = baseRot.z + f.a * 0.3 * (i % 3 === 0 ? 1 : -1);
    m.scale.y = 1 - f.a * 0.26;
    m.scale.x = 1 + f.a * 0.16;
    m.scale.z = 1 + f.a * 0.16;
    (m.material as THREE.MeshBasicMaterial).opacity = Math.min(
...
function buildScene() {
  ...
  // melt-01: transform bases for the ooze (position/rotation/scale home)
  ...
  // melt01.luma.stats). A 2D canvas tap is synchronous and honest: draw
...
export async function mountRuntime(
  ...
  return await sketchEvents.measured("study", "study.mount", { study: STUDY }, async () => {
    ...
    const maybeOpen = (elapsed: number): void => {
      ...
      const vid = vidOf();
      // r5b: the shutter requires AT LEAST ONE DECODED FRAME. On a cold
      // cache the element reaches the seek position while still
      // readyState 1 (metadata only): opening there produced the
      // black-screen aperture and let the capture budget expire before
      // any frame could be drawn.
      if (!vid || vid.readyState < 2 || Math.abs(vid.currentTime - SEEK_TO) > 1.5) return;
    ...
    const openAperture = (at: number, via: "seeked" | "backstop"): void => {
      ...
      // melt-01 v01: the plate rides GREY luma only — the melt is the one
      // operation; no channel split rides along (colour lane stays parked
      // even though the cut pause is lifted).
      ...
      void sketchEvents
        .emitInfo("study", "melt01.aperture", { at, via })
        .catch(() => undefined);
    };
    ...
    const onSeeked = (ev: Event): void => {
      ...
      if (vid.readyState < 2) return; // r5b: no frame yet, not actually armed
      if (armedAtMs === null) { armedAtMs = performance.now(); armedVia = "seeked"; }
      void sketchEvents
        .emitInfo("study", "melt01.armed", { at: Number(vid.currentTime.toFixed(2)) })
        .catch(() => undefined);
    };
    ...
    const revealProofTick = (elapsed: number): void => {
      ...
      const vid = vidOf();
      void sketchEvents
        .emitInfo("study", "melt01.arm.proof", {
    ...
    const revealBackstop = (): void => {
      ...
      const vid = vidOf();
      if (!vid) return;
      // r5: readyState>=2 means at least one decoded frame exists. The
      // old backstop keyed on seek-position metadata only (readyState 1),
      // which opened the aperture onto a black screen on cold caches and
      // let an 8s capture budget expire before any frame existed.
    ...
    // aperture (wall clock). A freshly-swapped video element can sit at
    // readyState 1 (seek done, no decoded frame) past that whole window on
    // a cold cache — receipt: melt01.arm.proof readyState 1 at elapsed
    // 8007ms, wall proof mean 7 (initial canvas), heap rendered black.
    // The budget now counts only time in which a capture was POSSIBLE.
    let capturePossiblyAtMs: number | null = null;
    let bestWallMean = 0;
    ...
    return {
      ...
      onPointer(p) {
        ...
        void sketchEvents
          .emitInfo("study", "melt01.intervention", { sha: p.sha.slice(0, 16), source: "pointer" })
          .catch(() => undefined);
      },
      step(r, now, delta) {
        ...
        if (tap) {
          ...
          if (lumaFramesSeen % 600 === 1) {
            ...
            void sketchEvents
              .emitInfo("study", "melt01.luma.stats", {
                seen: lumaFramesSeen,
          ...
          if (detector.push(frame)) {
            study.traceAt = performance.now();
            void sketchEvents
              .emitInfo("study", "melt01.cut", { at: Number(study.traceAt.toFixed(0)), source: "film" })
              .catch(() => undefined);
          }
        }
        ...
        if (!railAudit.reported && elapsed > 48_000) {
          ...
          void sketchEvents
            .emitInfo("study", "melt01.rail.audit", {
        ...
        if (bubbleLogs < 10) {
          ...
          if (Math.floor(elS) > bubbleLogs) {
            ...
            void sketchEvents
              .emitInfo("study", "melt01.beam.alpha", {
                s: bubbleLogs,
                cam: [
        ...
        if (openMs >= 3_000 && snapProof.count < 3 && (snapProof.count === 0 || openMs >= (snapProof.count + 1) * 10_000)) {
          ...
          void sketchEvents
            .emitInfo("study", "melt01.wall.proof", { sample: snapProof.count, mean: Number((acc / cnt).toFixed(2)) })
            .catch(() => undefined);
        }
        ...
        const vwNow = vidOf();
        const captureReady = (vwNow?.readyState ?? 0) >= 2;
        if (captureReady && capturePossiblyAtMs === null) capturePossiblyAtMs = now;
        const captureElapsed = capturePossiblyAtMs === null ? 0 : now - capturePossiblyAtMs;
        ...
        } else if (openMs >= 8_000 && captureElapsed >= 8_000 && wallMean < 20 && bestWallData && !bestCommitted) {
        ...
        if (!wallProofEmitted && openMs >= 3100) {
          ...
          void sketchEvents
            .emitInfo("study", "melt01.wall.mean", { mean: Number((wacc / wcnt).toFixed(2)) })
            .catch(() => undefined);
        }
